...
public class FileModification
{
    ...
    public IPatchFileResource? ResourceEntry { get; }
    ...
    private void AddAddedFileToHistory(LibraryFileHistoryBuilder historyBuilder, bool fileExists, Func<Stream> getCurrentFile)
    {
        ...
        {
            historyBuilder.AddAddedFile(PatchFilePath);
        }
        ...
        {
            IPatchFileResource historyResourceEntry = HistoryEntry?.ResourceEntry ?? throw new Exception("Missing history resource entry");

            historyBuilder.AddReplacedFile(PatchFilePath, historyResourceEntry);
        }
        ...
        else
        {
            if (fileExists)
            {
                ...
                historyBuilder.AddReplacedFile(PatchFilePath, new VirtualPatchFileResource(PatchFilePath, fileStream));
            }
            else
            {
                historyBuilder.AddAddedFile(PatchFilePath);
    ...
    private void AddRemovedFileToHistory(LibraryFileHistoryBuilder historyBuilder, bool fileExists, Func<Stream> getCurrentFile)
    {
        ...
        else if (HistoryEntry?.Type is HistoryFileType.Replace or HistoryFileType.Remove)
        {
            ...
            historyBuilder.AddRemovedFile(PatchFilePath, historyResourceEntry);
        }
        ...
        {
            using Stream fileStream = getCurrentFile();
            historyBuilder.AddRemovedFile(PatchFilePath, new VirtualPatchFileResource(PatchFilePath, fileStream));
        }
    }
        ...
        Action deleteFile)
    {
        if (Type == FileType.Add)
        {
            ...
            IPatchFileResource resourceEntry = ResourceEntry ?? throw new Exception("Missing resource entry");
    ...
    public record HistoryFileEntry(HistoryFileType Type, IPatchFileResource? ResourceEntry = null);
