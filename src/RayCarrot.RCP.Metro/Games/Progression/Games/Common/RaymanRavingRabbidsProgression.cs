...
public static class RaymanRavingRabbidsProgression
{
    ...
    public static IReadOnlyList<GameProgressionDataItem> CreateStoryProgressionItems(RRR_SaveSlot slot)
    {
    ...
    public static IReadOnlyList<GameProgressionDataItem> CreateScoreProgressionItems(
        RRR_SaveSlot slot,
        ...
        out int maxCollectiblesCount)
    {
        ...
        for (int i = 0; i < 128; i++)
        {
            ...
            int[] playerScores = Enumerable.Range(0, 3).Where(i => slot.Univers.MG_record_pn[mgID * 3 * 3 + i * 3] > 0).ToArray();
            ...
            int highScoreIndex = isAscending
                ? playerScores.OrderBy(i => slot.Univers.MG_record[mgID * 3 + i]).Last()
                : playerScores.OrderByDescending(i => slot.Univers.MG_record[mgID * 3 + i]).Last();

            float playerHighScore = slot.Univers.MG_record[mgID * 3 + highScoreIndex];
            ...
            string char0 = getChar(slot.Univers.MG_record_pn[mgID * 3 * 3 + highScoreIndex * 3 + 0]);
            string char1 = getChar(slot.Univers.MG_record_pn[mgID * 3 * 3 + highScoreIndex * 3 + 1]);
            string char2 = getChar(slot.Univers.MG_record_pn[mgID * 3 * 3 + highScoreIndex * 3 + 2]);
        ...
        collectiblesCount = totalScore;
        maxCollectiblesCount = maxScore;

        return scoreDataItems;
    }
