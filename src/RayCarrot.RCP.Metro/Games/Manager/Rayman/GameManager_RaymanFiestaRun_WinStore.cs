...
public sealed class GameManager_RaymanFiestaRun_WinStore : GameManager_WinStore
{
    ...
    protected override Task<bool> IsDirectoryValidAsync(FileSystemPath installDir, object parameter = null)
    {
        ...
        if (!((installDir + Game.GetGameDescriptor<GameDescriptor_RaymanFiestaRun_WindowsPackage>().GetFiestaRunFileName(parameter as UserData_FiestaRunEdition? ?? Services.Data.Game_FiestaRunVersion)).FileExists))
            return Task.FromResult(false);
