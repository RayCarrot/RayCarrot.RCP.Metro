    ...
    public class RayGameInstaller : IStatusUpdated
    {
        ...
        public virtual async Task<RayGameInstallerResult> InstallAsync()
        {
            ...
            try
            {
                ...
                RCFRCP.File.DeleteDirectory(temp);
                ...
                if (InstallData.OutputDir.DirectoryExists)
                {
                    ...
                    if (existingPaths.Count > 0)
                    {
                        ...
                        foreach (FileSystemPath path in existingPaths)
                            // Move file
                            RCFRCP.File.MoveFile(path, temp + path.GetRelativePath(InstallData.OutputDir), false);
                    }
                }
            ...
            finally
            {
                ...
                if (!complete)
                {
                    ...
                    if (fileConflicts)
                    {
                        try
                        {
                            RCFRCP.File.CopyDirectory(temp, InstallData.OutputDir, false, true);
                        }
                        catch (Exception ex2)
                        {
