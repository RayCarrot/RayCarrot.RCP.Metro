...
namespace RayCarrot.RCP.Metro
{
    ...
    public class RLLocalizationConverterUtilityViewModel : BaseUbiArtLocalizationConverterUtilityViewModel<UbiArtLocStringValuePair[]>
    {
        ...
        {
            return BinarySerializableHelpers.ReadFromFile<UbiArtLocalizationData>(file, UbiArtSettings.GetDefaultSettings(UbiArtGame.RaymanLegends, Platform.PC), RCPServices.App.GetBinarySerializerLogger(file.Name)).Strings;
        }
        ...
        protected override void Serialize(FileSystemPath file, UbiArtLocStringValuePair[] data)
        {
            ...
            var currentData = BinarySerializableHelpers.ReadFromFile<UbiArtLocalizationData>(file, UbiArtSettings.GetDefaultSettings(UbiArtGame.RaymanLegends, Platform.PC), RCPServices.App.GetBinarySerializerLogger(file.Name));

            // Replace the string data
            currentData.Strings = data;

            // Serialize the data
            BinarySerializableHelpers.WriteToFile(currentData, file, UbiArtSettings.GetDefaultSettings(UbiArtGame.RaymanLegends, Platform.PC), RCPServices.App.GetBinarySerializerLogger(file.Name));
        }
    }
}