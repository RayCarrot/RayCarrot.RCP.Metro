            ...
            foreach (var game in GameItems)
            {
                ...
                foreach (var action in game.Value)
                {
                    ...
                    try
                    {
                        ...
                        if ((result.Type == GameType.Win32 || result.Type == GameType.DosBox || result.Type == GameType.EducationalDosBox) && !await game.Key.GetManager(result.Type).IsValidAsync(result.Path))
                        {
